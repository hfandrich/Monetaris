...
export const Tenants: React.FC = () => {
  ...
  const getTenantStats = (tenantId: string): TenantStats => {
    const tenantCases = cases.filter(c => c.tenantId === tenantId);
    const activeVolume = tenantCases.reduce((sum, c) => sum + c.totalAmount, 0);
    const uniqueDebtors = new Set(tenantCases.map(c => c.debtorId)).size;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredTenants.map((tenant) => {
            ...
            return (
              ...
              >
                 <div className="flex justify-between items-start mb-8 cursor-pointer" onClick={() => navigate(`/tenants/${tenant.id}`)}>
                 ...
                 <div className="flex items-center gap-3 pt-6 border-t border-slate-100 dark:border-white/5">
                    ...
                    <Button onClick={() => navigate(`/tenants/${tenant.id}`)} variant="ghost" size="sm" className="ml-auto text-monetaris-600 dark:text-monetaris-400 hover:bg-monetaris-50 dark:hover:bg-monetaris-500/10">
