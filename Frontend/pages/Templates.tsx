...
import { CommunicationTemplate, TemplateVariable, CollectionCase, Debtor, Tenant } from '../types';
import { Plus, Save, Eye, EyeOff, FileText, Mail, Braces, Bold, Italic, Underline, AlignLeft, AlignCenter, AlignRight, List, Heading1, Heading2, Undo, Redo, Check, AlertTriangle, User, Printer, Wand2 } from 'lucide-react';
...
const VARIABLES: { category: string; items: TemplateVariable[] }[] = [
  ...
  },
  {
    ...
    items: [
      { key: 'tenant.name', label: 'Name', category: 'Tenant', example: 'TechCorp AG' },
      { key: 'tenant.bankAccountIBAN', label: 'IBAN', category: 'Tenant', example: 'DE99 ...' },
      { key: 'tenant.contactEmail', label: 'E-Mail', category: 'Tenant', example: 'billing@techcorp.de' },
    ]
  },
  {
      category: 'System',
...
export const Templates: React.FC = () => {
  ...
  const [mockData, setMockData] = useState<{ debtor: Debtor, case: CollectionCase, tenant: Tenant } | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      const tpls = await dataService.getTemplates();
      setTemplates(tpls);
  ...
  const loadContext = async (c: CollectionCase) => {
     const debtorRes = await dataService.getDebtorById(c.debtorId);
     const tenantRes = await dataService.getTenantById(c.tenantId);
     if (debtorRes && tenantRes) {
         setMockData({
             debtor: debtorRes.debtor,
             case: c,
  ...
  const handleCreateNew = async (type: 'EMAIL' | 'LETTER') => {
      const newTpl = await dataService.createTemplate(`Neue Vorlage (${type === 'EMAIL' ? 'E-Mail' : 'Brief'})`, type);
  ...
  const handleSave = async () => {
      ...
      await dataService.saveTemplate(updated);
            ...
            <div class="page">
                <div class="header-fixed">
                    <div class="header-content">
                        ...
                        <div class="meta-block">
                            <strong>${context.tenant.name}</strong><br>
                ...
                <div class="content-wrapper">
                    ...
                    <div class="address-zone">
                        <div class="sender-tiny">${context.tenant.name} • Postfach 101 • 10115 Berlin</div>
                ...
                <div class="footer-fixed">
                    <div class="footer-content">
                        <div>
                            <strong>${context.tenant.name}</strong><br>
                            ${context.tenant.registrationNumber}<br>
                            Geschäftsführung: Dr. A. Monetaris
                        </div>
                        <div>
                            <strong>Kontakt</strong><br>
                            ${context.tenant.contactEmail}<br>
                            www.monetaris.com
                        </div>
                        <div style="text-align: right;">
                            <strong>Bankverbindung</strong><br>
                            IBAN: ${context.tenant.bankAccountIBAN}<br>
                              ...
                              <div className="h-auto border-b-2 border-slate-900 mb-8 flex justify-between items-end pb-4 select-none opacity-50 hover:opacity-100 transition-opacity cursor-default">
                                   <div className="flex-1">
                                       ...
                                       <div className="text-xs text-slate-500">
                                           <span className="font-bold">{mockData?.tenant.name || 'Musterfirma GmbH'}</span> • Inkasso-Service
                                       </div>
                                   </div>
                              ...
                              {selectedTemplate.type === 'LETTER' && (
                                  <div className="mb-12 text-sm relative select-none">
                                      <div className="w-[85mm] h-[40mm] mb-4 p-2 rounded hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200">
                                        <p className="text-[8px] text-slate-400 underline mb-2">{mockData?.tenant.name || 'Firma'} • Postfach 123 • 10115 Berlin</p>
