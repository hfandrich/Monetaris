...
import { Kreditor, CollectionCase, User, CaseStatus, ImportProviderType, ImportMapping } from '../types';
...
export const KreditorDetail: React.FC = () => {
    ...
    const load = async () => {
      if (id) {
        try {
          setLoading(true);
          setError(null);
          const kreditorData = await kreditorenApi.getById(id);
          ...
          setKreditor(kreditorData);
          setCases(casesData.data);
          setFilteredCases(casesData.data);
          setUsers(fullData?.users || []);
        } catch (err) {
          const apiError = err as ApiError;
          setError(apiError.message || 'Failed to load kreditor data');
          console.error('Error loading kreditor:', err);
        } finally {
          setLoading(false);
        }
      }
  ...
  const executeImport = async () => {
      if(!tenant) return;
      setIsImporting(true);
      await dataService.processImport(kreditor.id, mappings, importProvider);
  ...
  if (loading) return <div className="p-12 text-center text-slate-500 animate-pulse">Lade Kreditorakte...</div>;
  if (error) return (
    <div className="p-12 text-center">
      <div className="glass-panel p-12 rounded-[32px] border-2 border-red-200 dark:border-red-500/20 bg-red-50 dark:bg-red-500/5 max-w-2xl mx-auto">
        ...
        <Button variant="secondary" onClick={() => navigate('/kreditoren')}>
          <ArrowLeft size={18} className="mr-2" /> Zurück zur Übersicht
        </Button>
      </div>
    </div>
  );
  if (!tenant) return <div className="p-12 text-center">Kreditor nicht gefunden.</div>;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <Button variant="ghost" onClick={() => navigate('/kreditoren')} className="mb-4 pl-0 hover:bg-transparent">
        <ArrowLeft size={18} className="mr-2" /> Zurück zur Übersicht
      </Button>
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
             <div className="flex items-start md:items-center gap-4 mb-4">
                ...
                <div>
                    <h1 className="text-2xl md:text-4xl font-display font-bold text-slate-900 dark:text-white break-all">{kreditor.name}</h1>
                    <div className="flex items-center gap-3 mt-2">
                         <Badge color="green">Aktiv</Badge>
                         <span className="font-mono text-xs text-slate-500">{kreditor.registrationNumber}</span>
                    </div>
                </div>
             </div>
        </div>
                 ...
                 <Card className="dark:bg-[#0A0A0A]">
                    ...
                    <div className="space-y-6">
                         <div className="flex flex-col sm:flex-row sm:justify-between py-3 border-b border-slate-100 dark:border-white/5 gap-1">
                             ...
                             <span className="text-sm font-bold text-slate-900 dark:text-white">{kreditor.name}</span>
                         </div>
                         <div className="flex flex-col sm:flex-row sm:justify-between py-3 border-b border-slate-100 dark:border-white/5 gap-1">
                             <span className="text-sm text-slate-500">Handelsregister</span>
                             <span className="text-sm font-bold text-slate-900 dark:text-white">{kreditor.registrationNumber}</span>
                         </div>
                         <div className="flex flex-col sm:flex-row sm:justify-between py-3 border-b border-slate-100 dark:border-white/5 gap-1">
                             <span className="text-sm text-slate-500">Kontakt E-Mail</span>
                             <span className="text-sm font-bold text-slate-900 dark:text-white">{kreditor.contactEmail}</span>
                         </div>
                         <div className="flex flex-col sm:flex-row sm:justify-between py-3 border-b border-slate-100 dark:border-white/5 gap-1">
                             <span className="text-sm text-slate-500">Auszahlungskonto (IBAN)</span>
                             <span className="text-sm font-mono font-bold text-slate-900 dark:text-white break-all">{kreditor.bankAccountIBAN}</span>
                         </div>
                    </div>
                 </Card>
