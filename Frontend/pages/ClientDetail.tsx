...
export const ClientDetail: React.FC = () => {
  ...
  const executeImport = async () => {
      if(!client) return;
      setIsImporting(true);
      await dataService.processImport(client.id, mappings, importProvider);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <Button variant="ghost" onClick={() => navigate('/clients')} className="mb-4 pl-0 hover:bg-transparent">
        <ArrowLeft size={18} className="mr-2" /> Zurück zur Übersicht
      </Button>
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
             <div className="flex items-start md:items-center gap-4 mb-4">
                ...
                <div>
                    <h1 className="text-2xl md:text-4xl font-display font-bold text-slate-900 dark:text-white break-all">{client.name}</h1>
                    <div className="flex items-center gap-3 mt-2">
                         <Badge color="green">Aktiv</Badge>
                         <span className="font-mono text-xs text-slate-500">{client.registrationNumber}</span>
                    </div>
                </div>
             </div>
        </div>
        <div className="flex flex-wrap gap-3 w-full lg:w-auto">
            ...
            <Button variant="glow" className="flex-1 lg:flex-none" onClick={() => setIsReportOpen(true)}>Bericht erstellen</Button>
        </div>
      </div>
                 ...
                 <Card className="dark:bg-[#0A0A0A]">
                    ...
                    <div className="space-y-6">
                         ...
                         <div className="flex flex-col sm:flex-row sm:justify-between py-3 border-b border-slate-100 dark:border-white/5 gap-1">
                             ...
                             <span className="text-sm font-bold text-slate-900 dark:text-white">{client.registrationNumber}</span>
                         </div>
                         <div className="flex flex-col sm:flex-row sm:justify-between py-3 border-b border-slate-100 dark:border-white/5 gap-1">
                             <span className="text-sm text-slate-500">Kontakt E-Mail</span>
                             <span className="text-sm font-bold text-slate-900 dark:text-white">{client.contactEmail}</span>
                         </div>
                         <div className="flex flex-col sm:flex-row sm:justify-between py-3 border-b border-slate-100 dark:border-white/5 gap-1">
                             <span className="text-sm text-slate-500">Auszahlungskonto (IBAN)</span>
                             <span className="text-sm font-mono font-bold text-slate-900 dark:text-white break-all">{client.bankAccountIBAN}</span>
                         </div>
                    </div>
                 </Card>
              ...
              <div className="animate-in slide-in-from-bottom-2 space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50 dark:bg-[#0A0A0A] p-2 rounded-2xl border border-slate-200 dark:border-white/10">
                      <div className="flex gap-2 px-2 items-center">
                          <span className="text-xs font-bold text-slate-500 uppercase tracking-wide pt-0.5 mr-1">Ansicht:</span>
                      ...
                      <Button size="sm" variant="secondary" onClick={() => navigate(`/claims?tenantId=${client.id}&view=BOARD`)} className="border-monetaris-200 bg-monetaris-50 text-monetaris-700 dark:bg-monetaris-500/10 dark:text-monetaris-400 dark:border-monetaris-500/30">
                          <LayoutGrid size={16} className="mr-2"/> Kanban Board öffnen
                      </Button>
