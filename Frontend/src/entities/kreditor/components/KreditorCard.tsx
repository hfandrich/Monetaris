...
interface KreditorCardProps {
  kreditor: Kreditor;
  onClick?: () => void;
  showStats?: boolean;
}

export function KreditorCard({ kreditor, onClick, showStats = true }: KreditorCardProps) {
  return (
    <div
      onClick={onClick}
      data-testid="kreditor-card"
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          ...
          <div className="bg-slate-50 dark:bg-[#151515] p-4 rounded-2xl border border-slate-100 dark:border-white/5">
            ...
            <p className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
              {kreditor.totalCases || 0} <ArrowRight size={12} className="text-slate-400" />
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3 mb-8">
        ...
        {showStats && (
          <div className="flex items-center text-sm text-slate-600 dark:text-slate-300">
            ...
            {kreditor.totalDebtors || 0} Schuldner aktiv
          </div>
        )}
      </div>
