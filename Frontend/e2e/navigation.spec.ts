...
test.describe('Navigation and Routing', () => {
  ...
  test('Can navigate to all main sections', async ({ page }) => {
    ...
    // Kreditoren/Kreditors
    const kreditorenLink = page.locator('a[href*="#/kreditoren"], a[href*="#/kreditors"]').first();
    if (await kreditorenLink.isVisible()) {
      await kreditorenLink.click();
      await expect(page).toHaveURL(/.*#\/(kreditoren|kreditors)/);
    }
  ...
  test('404 page handling', async ({ page }) => {
    ...
    await page.goto(hashUrl('/this-does-not-exist-xyz'));

    // Should either show 404 or redirect to home
    // With HashRouter, unknown routes redirect to home based on App.tsx
